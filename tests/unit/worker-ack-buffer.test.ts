...
describe("WorkerAckBuffer", () => {
    let mockCompleteUseCase: {
        executeMany: MockedFunction<CompleteJobUseCase<unknown>["executeMany"]>;
    };
    let onCompleted: MockedFunction<(job: Job<unknown>) => void>;
    let onError: MockedFunction<(error: Error) => void>;
    let onBatchFlushed: MockedFunction<(count: number, durationMs: number) => void>;
    ...
    beforeEach(() => {
        mockCompleteUseCase = {
            executeMany: vi.fn().mockResolvedValue(undefined as never) as never,
        };
        onCompleted = vi.fn();
        onError = vi.fn();
        onBatchFlushed = vi.fn();
    });

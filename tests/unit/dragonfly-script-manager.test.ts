...
describe("DragonflyScriptManager", () => {
    ...
    it("should execute via evalsha when supported", async () => {
        const manager = DragonflyScriptManager.getInstance();
        const mockRedis = {
            evalsha: jest.fn().mockResolvedValue("result-123" as never),
        } as unknown as Redis;
    ...
    it("should reload script and retry evalsha on NOSCRIPT error", async () => {
        ...
        const mockRedis = {
            ...
            script: jest.fn().mockResolvedValue("OK" as never),
        } as unknown as Redis;
    ...
    it("should fallback to eval if script LOAD throws after NOSCRIPT", async () => {
        const manager = DragonflyScriptManager.getInstance();
        const mockRedis = {
            evalsha: jest.fn().mockRejectedValue(new Error("NOSCRIPT No matching script") as never),
            script: jest.fn().mockRejectedValue(new Error("SCRIPT LOAD failed") as never),
            eval: jest.fn().mockResolvedValue("eval-fallback-result" as never),
        } as unknown as Redis;
    ...
    it("should fallback to eval when evalsha fails with non-NOSCRIPT error or non-Error", async () => {
        const manager = DragonflyScriptManager.getInstance();
        const mockRedis1 = {
            evalsha: jest.fn().mockRejectedValue(new Error("SYNTAX ERROR") as never),
            eval: jest.fn().mockResolvedValue("eval-res1" as never),
        } as unknown as Redis;
        ...
        const mockRedis2 = {
            evalsha: jest.fn().mockRejectedValue("string-error" as never),
            eval: jest.fn().mockResolvedValue("eval-res2" as never),
        } as unknown as Redis;
    ...
    it("should fallback to eval when NOSCRIPT occurs but redis.script is not a function", async () => {
        ...
        const mockRedis = {
            evalsha: jest.fn().mockRejectedValue(new Error("NOSCRIPT No matching script") as never),
            eval: jest.fn().mockResolvedValue("eval-no-script-fn" as never),
        } as unknown as Redis;
    ...
    it("should execute directly via eval when evalsha is not a function", async () => {
        const manager = DragonflyScriptManager.getInstance();
        const mockRedis = {
            eval: jest.fn().mockResolvedValue("direct-eval-result" as never),
        } as unknown as Redis;
